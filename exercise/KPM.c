...
int* getNext(SString pattern){
    ...
    int Pnext[pattern.length+1];//Pnext是用来观测next数组的变化的
    Pnext[0]=-1;
    Pnext[1]=0;
    next[0]=-1;//next[0]废弃不用，因为要和模式串对齐,next[0]的值可以随便填。
...
int KMP(SString text,SString pattern){
    ...
    int i=1;//主串的下标
    int j=1;//模式串的下标
    while(i<=text.length&&j<=pattern.length){//i<=text.length&&j<=pattern.length说明主串和模式串都没有遍历完
